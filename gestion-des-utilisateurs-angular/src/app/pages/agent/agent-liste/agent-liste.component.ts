...
export class AgentListeComponent implements OnInit, OnDestroy {
  ...
  // recherche custom
  searchTermsFilterDoubleTelephoneEmail = new Subject<string>();
  termeRechercheTelephoneEmail: string = "";
  agentsFilterDoubleTelephoneEmail$: Observable<Agent[]> = of();
  /* ----------------------------------------------------------------------------------------- */
  ...
  ngOnInit(): void {
    ...
    this.agentsFilterDoubleTelephoneEmail$ = this.searchTermsFilterDoubleTelephoneEmail.pipe(
      // {...."ab"..."abz"."ab"...."abc"......}
      debounceTime(300),
      // {......"ab"...."ab"...."abc"......}
      distinctUntilChanged(),
  ...
  search(term: string): void {
    this.termeRechercheTelephoneEmail = term;
    this.searchTerms.next(term);
    this.searchTermsFilterDoubleTelephoneEmail.next(term);
  }
  ...
  FilterDoubleTelephoneEmail(termeRechercheTelephoneEmail: string) {
    this.termeRechercheTelephoneEmail = termeRechercheTelephoneEmail;
    this.myInputSearch.nativeElement.value = termeRechercheTelephoneEmail;
    this.dataSource.filter = termeRechercheTelephoneEmail.trim().toLowerCase(); // supprimer les espaces vide et mettre minuscule
    this.focusOnInput = false;
  }


  isNumber(termeRechercheTelephoneEmail: string): boolean {
    return !isNaN(Number(termeRechercheTelephoneEmail))
  }
